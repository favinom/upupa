...
#include "GrowthSolid_v2.h"

registerMooseObject("upupaApp", GrowthSolid_v2);

InputParameters
GrowthSolid_v2::validParams()
{
  InputParameters params = Kernel::validParams();
  	...
  	params.addParam<Real>("gamma_lys", 0.5, "tumor lys");
...
Real GrowthSolid_v2::computeQpResidual(){
...
Real effectStress = 1 - _delta1*posStress*traceStress/(3*_delta2 + posStress*traceStress);


    return _J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J[_qp] - _J_K[_qp]*_Phisv)/(_J[_qp]*(1-_Phisv)) - _gamma_lys/_rhoR*(1-_cp[_qp]) )*_test[_i][_qp];
}


Real GrowthSolid_v2::computeQpJacobian(){
...
Real GrowthSolid_v2::computeQpOffDiagJacobian(unsigned int jvar){
RealTensorValue H;
RealTensorValue Jac;
int component_H;
...
if(jvar == _id_JK){
 ...
 return ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J[_qp] - _J_K[_qp]*_Phisv)/(_J[_qp]*(1-_Phisv)) - _gamma_lys/_rhoR*(1-_cp[_qp]) -_J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*_Phisv/(_J[_qp]*(1-_Phisv)))   )*_test[_i][_qp]*_phi[_j][_qp];
    ...
    return _J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*(_J[_qp] - _J_K[_qp]*_Phisv)/(_J[_qp]*(1-_Phisv)) + _gamma_lys/_rhoR )*_test[_i][_qp]*_phi[_j][_qp];
    
}
else if(jvar==_id_omega){

	return _J_K[_qp]*( _gamma_upt/_rhoR*posNutrient/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J[_qp] - _J_K[_qp]*_Phisv)/(_J[_qp]*(1-_Phisv)))*_test[_i][_qp]*_phi[_j][_qp];
...
H(component_H,0) = _grad_phi[_j][_qp](0); H(component_H,1) = _grad_phi[_j][_qp](1); H(component_H,2) = _grad_phi[_j][_qp](2);
 	...
 	return _J_K[_qp]* _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*( _J_K[_qp]*_Phisv/_J[_qp])*FinvT.contract(H)/(1-_Phisv)*_test[_i][_qp];
