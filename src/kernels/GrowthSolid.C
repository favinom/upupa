...
#include "GrowthSolid.h"

registerMooseObject("upupaApp", GrowthSolid);

InputParameters
GrowthSolid::validParams()
{
...
GrowthSolid::GrowthSolid(const InputParameters & parameters) : 
Kernel(parameters),
...
Real GrowthSolid::computeQpResidual(){
...
if(_J_old[_qp] - _J_K[_qp]*_Phisv < 0){
std::cout << "Orrore nel residuo della crescita della fase solida" << std::endl;
}

    return _J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J_old[_qp] - _J_K[_qp]*_Phisv)/(_J_old[_qp]*(1-_Phisv)) - _gamma_lys/_rhoR*(1-_cp[_qp]) )*_test[_i][_qp];
}


Real GrowthSolid::computeQpJacobian(){
...
Real GrowthSolid::computeQpOffDiagJacobian(unsigned int jvar){
RealTensorValue H;
RealTensorValue Jac;
int component_H;
...
Real effectStress = 1 - _delta1*posStress*traceStress/(3*_delta2 + traceStress);
 ...
 return ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J_old[_qp] - _J_K[_qp]*_Phisv)/(_J_old[_qp]*(1-_Phisv)) - _gamma_lys/_rhoR*(1-_cp[_qp]) -_J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*_Phisv/(_J_old[_qp]*(1-_Phisv)))   )*_test[_i][_qp]*_phi[_j][_qp];
    ...
    return _J_K[_qp]* ( _gamma_upt/_rhoR*posNutrient*(_omega[_qp]-_omega_cr)/(_omega_env-_omega_cr)*effectStress*(_J_old[_qp] - _J_K[_qp]*_Phisv)/(_J_old[_qp]*(1-_Phisv)) + _gamma_lys/_rhoR )*_test[_i][_qp]*_phi[_j][_qp];
	...
	return _J_K[_qp]*( _gamma_upt/_rhoR*posNutrient/(_omega_env-_omega_cr)*effectStress*_cp[_qp]*(_J_old[_qp] - _J_K[_qp]*_Phisv)/(_J_old[_qp]*(1-_Phisv)))*_test[_i][_qp]*_phi[_j][_qp];
